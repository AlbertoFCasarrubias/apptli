...
export class ScheduleComponent implements AfterContentChecked, OnDestroy, OnChanges {
    ...
    changeDrop(id) {
        if (this.user.doctor) {
            ...
            const hour = cols[1];
            const doctors = this.data.users.filter(u => u.adminID === this.data.user.patient);
    ...
    async showNewEventAlert(day, hour) {
        if (this.data.user.doctor) {
            ...
            const momentStart = moment(day + '@' + hour, 'YYYY-MM-DD@H');
            const momentEnd = moment(day + '@' + hour, 'YYYY-MM-DD@H').add(1, 'h');
            const payload = {
                createdBy: this.data.user.mail,
                duration: 60,
                hourEnd: momentEnd.format('HH:mm'),
                hourStart: momentStart.format('HH:mm'),
                patient: [this.data.user.id],
                start: momentStart.toISOString(),
                status: null,
                users: [this.data.users.find(u => u.adminID === this.data.user.patient).id]
            };

            await this.presentLoading();
            this.store.dispatch(new AddEvent(payload)).toPromise()
                .then(() => {
                    this.dismisLoading();
                    console.log('evento guardardo');
                })
