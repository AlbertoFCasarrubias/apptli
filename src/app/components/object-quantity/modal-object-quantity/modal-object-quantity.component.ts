...
export class ModalObjectQuantityComponent implements OnInit {
  ...
  check(item)
  {
    var find =  this.dataSelected.find(el =>
    {
      if(el && item && el._id == item._id)
      {
        return true;
      }
      else {
  ...
  selectItem(item)
  {
    ...
    if(find)
    {
      this.dataSelected = this.dataSelected.filter(el => el._id != item._id)
    }
    else
    {
      if(this.multiple)
  ...
  close(){
    this.dataSelected = this.navParams.data.initial;
    console.log('close ', this.dataSelected);
